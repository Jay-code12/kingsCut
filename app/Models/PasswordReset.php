...
class PasswordReset
{
    ...
    public static function generate(int $customerId): string
    {
        ...
        // Invalidate any previous unused codes for this customer
        $stmt = $db->prepare('UPDATE password_resets SET used_at = NOW() WHERE customer_id = ? AND used_at IS NULL');
        ...
        $stmt = $db->prepare(
            'INSERT INTO password_resets (customer_id, otp_hash, expires_at) VALUES (?, ?, ?)'
        );
    ...
    public static function verify(int $customerId, string $otpCode): string
    {
        ...
        if (!hash_equals($reset['otp_hash'], hash('sha256', $otpCode))) {
            $stmt = $db->prepare('UPDATE password_resets SET attempts = attempts + 1 WHERE id = ?');
