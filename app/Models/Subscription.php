...
class Subscription
{
     ...
     */
    public static function findByMembershipId(string $membershipId): ?array
    {
        $stmt = Database::getInstance()->prepare(
            'SELECT s.*, p.name AS plan_name, p.discount_percent
             FROM subscriptions s JOIN plans p ON p.id = s.plan_id
             WHERE s.membership_id = ? LIMIT 1'
        );
        $stmt->execute([strtoupper(trim($membershipId))]);
        return $stmt->fetch() ?: null;
    }
