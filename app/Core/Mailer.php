...
<<<<<<< HEAD
    ...
    public static function sendVerificationOtp(string $toEmail, string $firstName, string $otpCode): bool
    {
        $body = self::otpBody(e($firstName), $otpCode, 'verify your email for your new King&rsquo;s Cut Saloon account');
...
<<<<<<< HEAD
    ...
    {
        $body = self::broadcastBody(e($firstName), $message);
        return self::send($toEmail, $subject, self::shell($subject, $body));
    }
...
<<<<<<< HEAD
    ...
    {
        $ink = self::BRAND_INK;
        // The message is admin-authored plain text — escape it, then turn line
        // breaks into <br> so paragraphs the admin typed are preserved.
        $safeMessage = nl2br(e($message));
        return <<<HTML
        <p style="margin:0 0 16px; font-size:15px; color:{$ink}; line-height:1.6;">Hi {$firstName},</p>
        <p style="margin:0; font-size:15px; color:{$ink}; line-height:1.7;">{$safeMessage}</p>
        <p style="margin:24px 0 0; font-size:13px; color:#8A7A5C; line-height:1.6; font-family:Arial, sans-serif;">
